...
module var_first_step
    ...
    real(DP),parameter::a_min=0.0d0,a_max=5000.0d0,r=(1.0d0+0.02d0)**2.0d0-1.0d0!(1.0d0+0.02d0)**2.0d0-1.0d0
...
module initial_p
    ...
    implicit none
    !initial guess of parameters
    real(DP),dimension(G_DF)::betas_ini=0.98 !(/0.9d0,0.98d0/)
    real(DP)::beq_cur_ini=500.3d0 ,c_floor_ini=3.12d0,beq_mu_ini=5.0d0,RRA_ini=2.5d0
    real(DP),dimension(G_types,G_educ)::pr_betas_ini=1.0d0!reshape((/0.5,0.6,0.8,0.3,0.4,0.6,0.2,0.5,0.7/),shape(pr_betas_ini))
end module
