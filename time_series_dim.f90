...
!        end function
!        subroutine direct_effect(p_in,p_out,V_in,V_out)
!            use nrtype; use preference_p
!            implicit none
!            real(DP),dimension(:),intent(in)::p_in,p_out
...
!    joint_pr_bmk=joint_pr_model !joint_pr_bmk(:,:,:,2)
!    
!    !compute direct effect
!    joint_pr_bmk_de=joint_pr_bmk 
!    call direct_effect(p_fs,p_fs,av_V_ini_all(:,:,:,2),av_V_ini_all(:,:,:,4))
!    joint_pr_bmk_de(:,:,:,4)=joint_pr_model(:,:,:,4) !joint_pr_bmk_de(:,:,:,4)
!    
!    do c_l=1,3
!    
!        if (c_l==1) then
...
!    call direct_effect(p_fs,p_fs_new,av_V_ini_all(:,:,:,2),av_V_ini_all(:,:,:,4))
!
