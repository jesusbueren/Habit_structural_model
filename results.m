...
data_mat=data.data
data_mat=reshape(data_mat(:,2:5),3,3,2,4)
...
elseif j==2
    ...
    b=bar(X,[squeeze(data_mat(:,e_l,df_l,j:j+1)) counterfactual(:,e_l,df_l,j)]./sum([squeeze(data_mat(:,e_l,df_l,j:j+1)) counterfactual(:,e_l,df_l,j)]),'FaceColor','flat') 
    for c_l=1:3
